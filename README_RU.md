...
Для максимально предсказуемой и легкой обработки можно запускать собственный OpenCV-узел выделения/трекинга признаков и передавать результаты в контур управления.
...
source /opt/ros/humble/setup.bash
source ~/usv_ws/install/setup.bash
# Запуск вашего bridge/select/control узла
```
